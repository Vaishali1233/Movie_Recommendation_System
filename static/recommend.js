$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '69d8de85c5a25979d189cf20e7983c45';
    var title = $('.movie').val();
    if (title=="") {
...
function get_individual_cast(movie_cast,my_api_key) {
    ...
    for(var cast_id in movie_cast.cast_ids){
      $.ajax({
        type:'GET',
        url:'https://developers.themoviedb.org/person/'+movie_cast.cast_ids[cast_id]+'?api_key='+my_api_key,
        async:false,
...
function get_movie_cast(movie_id,my_api_key){
    ...
    $.ajax({
      type:'GET',
      url:"https://developers.themoviedb.org/3/movie/"+movie_id+"/credits?api_key="+my_api_key,
      async:false,
...
function get_movie_posters(arr,my_api_key){
  ...
  for(var m in arr) {
    $.ajax({
      type:'GET',
      url:'https://developers.themoviedb.org/3/search/movie?api_key='+my_api_key+'&query='+arr[m],
      async: false,
      success: function(m_data){
        arr_poster_list.push('https://www.themoviedb.org/t/p/original'+m_data.results[0].poster_path);
      },
      error: function(){
        alert("Invalid Request!");
        $("#loader").delay(500).fadeOut();
